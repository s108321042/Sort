...
def sort():  
 ...
 for i in range (1,len(arr)):
     ...
     for k in range(len(arr)):
         ...
         data = tk.Label(window, text=D,background="skyblue")
         data.pack()
         data.place(x=150+k*40,y=60+i*40)

         temp='第'+num[count]+'輪排序結果:'
         numlist= tk.Label(window,text=temp,background="skyblue")
         numlist.pack()
         numlist.place(x=5,y=60+i*40)

         if(k==len(arr)-1): break
         comma = tk.Label(window,text=',',background="skyblue")
         comma.pack()
         comma.place(x=175+40*k,y=60+i*40)

 for k in range(len(arr)):
         D=arr[k]
         data = tk.Label(window, text=D,background="skyblue")
         data.pack()
         data.place(x=150+k*40,y=60+(i+1)*40)
         if(k==len(arr)-1): break
         comma = tk.Label(window,text=',',background="skyblue")
         comma.pack()
         comma.place(x=175+40*k,y=60+(i+1)*40)

 last = tk.Label(window,text='最終排序結果：',background="skyblue")
 last.pack()
 last.place(x=5,y=60+(i+1)*40)
